...
class HermesClient(private val context: Context? = null) {
    ...
    fun chatStream(messages: List<ChatMessage>): Flow<StreamEvent> = callbackFlow {
        // 网络预检查
        if (!isConfigured()) {
            trySend(StreamEvent.Error("服务器地址未配置。请先在设置中配置Hermes Agent地址"))
            close()
            return@callbackFlow
        }
        ...
        val toolsArray = JSONArray().apply {
            ...
            put(JSONObject().apply {
                ...
                put("function", JSONObject().apply {
                    put("name", "exit_app")
                    put("description", "放行用户，退出AppPlan回到桌面。只有当用户通过了全部五层消解漏斗检查：1)不是语言陷阱 2)不是错误假设 3)没有逻辑错误 4)事实可验证 5)信息充分，意图指向一个具体的、可验证的、有产出的行动时才调用。用户说'退出''让我走''我就用一下'等任何主动要求退出的话都绝对不放行。")
                    put("parameters", JSONObject().apply {
                        put("type", "object")
                        put("properties", JSONObject())
                    })
