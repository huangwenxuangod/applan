...
applan 是一个 Android 应用，在你每次解锁手机后弹出，通过 AI 对话审核你的使用意图。只有当你能清晰、具体、可验证地说出要做什么时才放行，否则直接锁屏——帮你对抗无意识刷手机的冲动。
...
- **🏠 默认Launcher模式**：设为默认桌面，Home键直接回到applan
...
```
android/app/src/main/java/com/applan/
├── MainActivity.kt              # 全屏沉浸Activity
├── ApplanApp.kt                 # Application (DeviceProtectedStorage)
├── network/
│   ├── ApplanClient.kt          # SSE流式AI对话
│   ├── ChatMessage.kt           # 消息模型
│   └── StreamEvent.kt           # 流式事件
├── service/
...
applan 通过 SSE 连接到 applan 后端（默认 `http://<服务器IP>:8787`）。需要在 applan 后端：

1. 启动 applan 后端，确保监听 `0.0.0.0:8787`
2. 在 applan 设置页面配置服务器地址和 API Key
